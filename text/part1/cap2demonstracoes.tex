...
Em termos do diagrama, a prova de uma generalização começa inserido na primeira linha de um bloco a informação de que $x$ é um objeto genérico (ou qualquer) do discurso, em seguida é aberto um novo bloco para a subprova em que será provado que a asserção $P(x)$ é verdadeira, com o fechamento do bloco da subprova pode-se então concluir que $P(x)$ é verdadeiro para um $x$ genérico do discurso, após isso o bloco externo é fechado e  pode-se apresentar a conclusão de que todo objeto $x$ do discurso $P(x)$ é verdadeiro. Note que esse raciocínio de demonstração garante (com explicado em \cite{velleman2019comProvar}) que a asserção $P$ é universal sobre o universo do discurso, ou seja, garante a universalidade da asserção $P$.
...
\begin{definition}[Prova de existência (PE)]
	Para provar uma asserção da forma: ``$(\exists x).[P(x)]$'' é um asserção sobre a variável x. Deve-se exibir um elemento específico $a$ pertencente ao universo do discurso do enunciado, e mostrar que a asserção $P(x)$ é verdadeira quando $x$ é instância como sendo exatamente o elemento $a$, ou seja, deve-se mostrar que $P(a)$ é verdadeira.
\end{definition}

Em relação ao diagrama de bloco, uma demonstração de existência, isto é, uma prova de uma asserção $(\exists x).[P(x)]$,  irá se comportar de forma muito semelhante a uma demonstração de generalidade, as única mudanças significativas é que tal método inicia seu bloco com a declaração de que será atribuído um objeto \textbf{específico} em vez de considerar a variável genérica a $x$, além disso, a conclusão do bloco externo deve ser exatamente a $(\exists x).[P(x)]$, ou seja, a conclusão deverá ser a asserção existencial.
...
\begin{example}\label{exe:DiagramaProva18}
	...
	{\scriptsize
		\begin{logicproof}{3}
			\begin{subproof}
				\text{\textbf{Deixe ser} } a = \sqrt{2} \text{ e } b = \sqrt{2}&  --- Hipótese\\
			...
			\end{subproof}
			\text{\textbf{Portanto},} (\exists m, n \in \mathbb{I})[m^n \in \mathbb{Q}]. & --- Conclusão da PG (linhas $1$-$10$)
		\end{logicproof}
	}
\end{example}