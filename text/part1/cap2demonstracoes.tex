...
\begin{example}\label{exe:DiagramaProva16}
	Demonstração da asserção: $\forall n \in \mathbb{Z}.$[se $n > 2$, então $x^2 > x + x$].
	{\scriptsize
		\begin{logicproof}{3}
			\begin{subproof}
				\text{\textbf{Assuma} que } x \text{ é um número inteiro,}&  --- Hipótese\\
				\begin{subproof}
					\text{\textbf{Suponha} que } x > 2, & --- Hipótese\\
					\text{\textbf{logo} } x \cdot x > 2x,& --- Monotonicidade da multiplicação em $\mathbb{Z}$\\
					\text{\textbf{então} } x^2 > x + x.& --- Reescrita da linha $3$
				\end{subproof}
				\text{\textbf{Dessa forma}, se } n > 2, \text{ então } x^2 > x + x. & --- Conclusão  das PD (linhas $2$-$4$)
			\end{subproof}
			\text{\textbf{Portanto},} \forall n \in \mathbb{Z}.\text{[se $n > 2$, então $x^2 > x + x$]}. & --- Conclusão da PG (linhas $1$-$5$)
		\end{logicproof}
	}
\end{example}