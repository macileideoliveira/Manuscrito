...
\begin{definition}[Símbolos dos conjuntos numéricos]\label{def:SimbolosConjuntos}
	O conjunto dos números naturais\footnote{Neste manuscrito é considerado que o conjuntos dos naturais corresponde ao conjunto $\{0, 1, 2, \cdots\}$.}, inteiros, racionais, irracionais, reais e complexos são representados respectivamente por   $\Nat$, $\Int$,  $\Rac$,  $\Irr$,  $\Rea$ e  $\Com$.
\end{definition}

\begin{remark}
	Neste manuscrito $\Nat_*, \Int_*, \Rac_*$ e $\Rea_*$ irão denotar receptivamente o conjunto dos naturais, inteiros, racionais e reais sem o $0$. Já $\Int^+, \Rac^+$ e $\Rea^+$ irão denotar receptivamente o conjunto dos inteiros, racionais e reais positivos. E por fim, $\Int^-, \Rac^-$ e $\Rea^-$ irão denotar receptivamente o conjunto dos inteiros, racionais e reais negativos.
\end{remark}
...
\begin{example}\label{exe:ConjuntoHerdeiro}
	Dado o conjunto $\Int$ tem-se que o conjunto $N = \{x \mid x \in \Int \mbox{ e } x = 2k \mbox{ para algum } k \in \Int\}$ é claramente um subconjunto de $\Int$ pois todo número par é também um número inteiro.
\end{example}
...
\begin{example}\label{exe:Inclusao}
	...
	\begin{itemize}
		...
		\item[(c)] $\{x \mid x = 2k \mbox{ para algum } k \in \Nat\} \subseteq \Nat$.
...
\begin{example}\label{exe:Familia}
	Os conjuntos: 
	$$A_1 = \{\Int^*_+, \Int_-, \{\pi, \sqrt{-1}\}\} \mbox{ e } A_2 = \{\{a, b\}, \{\clubsuit, \spadesuit, \heartsuit, \lozenge\}, \Rea\}$$
	são ambos famílias.
\end{example}
...
\begin{example}\label{exe:UniaoDeConjuntos1}
	Dados os dois conjuntos $A = \{x \in \Nat \mid x = 2i \mbox{ para algum } i \in \Nat\}$ e $B = \{x \in \Nat \mid x = 2j + 1 \mbox{ para algum } j \in \Nat\}$ tem-se que $A \cup B = \Nat$.
\end{example}
...
\begin{example}\label{exe:IntersecaoDeConuuntos1}
	Dado $A_1 = \{x \in \Nat \mid x \mbox{ é múltiplo de } 2\}$ e $A_2 = \{x \in \Nat \mid x \mbox{ é múltiplo de } 3\}$ tem-se que $A_1 \cap A_2 = \{x \in \Nat \mid x \mbox{ é múltiplo de } 6\}$.
\end{example}
...
\begin{example}\label{exe:Complemento}
	Dado $P = \{ x \in \Int \mid x = 2k \mbox{ para algum } k \in \Int\}$ tem-se então o seguinte complemento $\overline{P} = \{ x \in \Int \mid x = 2k + 1 \mbox{ para algum } k \in \Int\}$.
\end{example}
...
\begin{example}\label{exe:DiferencaConjuntos2}
	Dado so conjuntos $\Int$ e $\Int_+^*$ tem-se que $\Int - \Int_+^* = \Int_-$.
\end{example}
...
\begin{example}
	Seja $D = \{\Int_+, \{0, -1, -2, -3\}, (\Int_- \cup \{0\})\}$, a interseção generalizada de $D$ corresponde ao conjunto $D_\cap = \{0\}$.
\end{example}
...
Uma propriedade interessante do conjuntos das partes como dito em \cite{lipschutz1978-TC}, é que se $A$ for da forma $A = \{x_1, \cdots, x_n\}$ para algum $n \in \Nat$, então pode-se mostrar que $\wp(A)$ terá exatamente $2^n$ elementos.
...
\begin{problem}\label{prob:Conjuntos6}
	Considerando o universo dos números naturais dado os subconjuntos:  $A = \{1, 2, 3, 4, 5\}$, $B = \{x \in \Nat \mid x^2 = 9\}$, $C = \{x \in \Nat \mid x^2 - 4x + 6 = 0\}$ e $D = \{x \in \Nat \mid  x = 2k \mbox{ para algum } k \in \Nat\}$, complemente as frase com os símbolos $\subseteq$ e $\not\subseteq$.
\end{problem}
...
\begin{exerList}
	...
	\item $C \ \underline{ \ \ \ \ \ \ } \ A$.
	\item $D \ \underline{ \ \ \ \ \ \ } \ B$.
	\item $B \ \underline{ \ \ \ \ \ \ } \ \Nat$.
	\item $\Nat \ \underline{ \ \ \ \ \ \ } \ D$.
	\item $A \ \underline{ \ \ \ \ \ \ } \ \Nat$.
	\item $A \ \underline{ \ \ \ \ \ \ } \ \Int_-$.
	\item $\Nat \ \underline{ \ \ \ \ \ \ } \ D$.
	\item $\Nat \ \underline{ \ \ \ \ \ \ } \ C$.
	\item $C \ \underline{ \ \ \ \ \ \ } \ \Nat$.
	\item $\{6\} \ \underline{ \ \ \ \ \ \ } \ C$.
\end{exerList}
...
\begin{exerList}
	...
	\item $\Nat \underline{ \ \ \ \ \ \ } \{2, 3, 6\}$.
	\item $\{\{\emptyset\}, \emptyset\} \underline{ \ \ \ \ \ \ } \{\{\{\emptyset\}, \emptyset\}, \emptyset\}$.
	\item $a \underline{ \ \ \ \ \ \ } \{\{a\}, b\} \underline{ \ \ \ \ \ \ } \{a, b, c\}$.
	\item $0 \underline{ \ \ \ \ \ \ }  \Int_+^*$.
	\item $\frac{1}{0} \underline{ \ \ \ \ \ \ }  \Rac$.
	\item $\emptyset \underline{ \ \ \ \ \ \ }  \emptyset$.
	\item $\{1, 2, 4\} \underline{ \ \ \ \ \ \ }  \{2, 4, 6\} \underline{ \ \ \ \ \ \ } \{y \mid y = 2x \mbox{ para algum } x \in \Nat\}$.
	\item $\{1\} \underline{ \ \ \ \ \ \ }  \Rea$.
	\item $\frac{3}{4} \underline{ \ \ \ \ \ \ }  \Nat$.
\end{exerList}
...
\begin{exerList}
	\item $\{\frac{2}{x} \mid x - 1 > 0 \mbox{ com } x \in \Nat\}$ não é subconjunto de $\Nat$. 
	\item $\{2, 3, 4, 6, 8\}$ não é subconjunto de $\{x \in \Nat \mid  x = 2k \mbox{ para algum } k \in \Nat\}$.
	\item $\{1, 2, 3\}$ é um subconjunto próprio do conjunto $\{1, 2, 3, 4, 5, 6, 7,8,9,0\}$.
	\item $\{0, 5\}$ é subconjunto de $\Int$ mas não é subconjunto de $\Int^*$.
	\item $\{ x \mid x + x = x\}$ é subconjunto próprio de $\Nat$.
	...
	\item O conjunto $\{x \in \Nat \mid 0 < x < 3\}$ tem exatamente 3 subconjuntos próprios.
\end{exerList}
