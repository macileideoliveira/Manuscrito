...
\begin{example}\label{exe:LinguagemLAFN1}
	O $\lambda$-AFN representado pelo grafo de transição da Figura \ref{fig:Linguagem-LAFN1}, reconhece a linguagem $L = \{w \in \{1,2,3\}^* \mid w = 1^i2^j3^k \text{ com } i,j,k \in \mathbb{N}\}$.
...
Uma classe particularmente interessante de autômatos são os chamados \textbf{autômatos com saída}, dos quais se destacam as máquinas de Mealy \cite{mealy1955} e Moore \cite{moore1956}, além de serem alicerces fundamentais da teoria dos autômatos finitos, tais máquina tem uma grande gama de aplicações práticas. Nesta seção será apresentada ao leitor uma formalização de ambas as máquinas de Moore, e será trabalhado a ideia de tais máquinas como tradutoras, além disso, será apresentado o relacionamento das máquinas de Mealy com os circuitos sequências. 
...
\begin{definition}[Máquina de Moore]\label{def:MooreMachine}
	Uma máquina (ou autômato) de Moore é uma estrutura $A = \langle Q, \Sigma, \Lambda, \delta, \Phi, q_0, F\rangle$ onde $Q, \Sigma, F$ e $q_0$ são da mesma forma que na Definição \ref{def:AFD}. Já $\delta: Q \times \Sigma \rightarrow Q$ é uma função parcial de transição e $\Lambda$ um alfabeto chamado \textbf{alfabeto de saída} e $\Phi: Q \rightarrow \Lambda^*$ é a função total de escrita dos estado da máquina de Moore.
\end{definition}

\begin{example}\label{exe:Moore1}
	A estrutura $M = \langle \{q_0, q_1, q_3\}, \{a, b\}, \{x, y, z\}, \delta, \Phi, q_0, \{q_1, q_2\} \rangle$ onde a função de transição é esboçada pela Tabela \ref{tab:TransicaoMoore1} a seguir, sendo a função de escrita dos estados é definida como: $\Phi(q_0) = xy, \Phi(q_1) = yz$ e $\Phi(q_2) = zx$. É uma máquina de Moore.
...
Novamente similar aos AFD pode-se utilizar a ideia de grafo de transição para representar visualmente as máquinas de Moore a única diferença que esta representação terá em relação aos grafos que representam os AFD, é que os vértices serão rotulados por pares $(q, w) \in Q \times \Lambda$ e $q_j$ em que $\Phi(q) = w$.
